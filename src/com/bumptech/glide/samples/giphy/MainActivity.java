...
/**
 * The primary activity in the Giphy sample that allows users to view trending
 * animated GIFs from Giphy's api.
 */
public class MainActivity extends Activity implements Api.Monitor {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		RecyclerViewPreloader<Api.GifResult> preloader = new RecyclerViewPreloader<>(
				Glide.with(this), adapter, preloadSizeProvider, 4);
	...
	private static class GifAdapter extends RecyclerView.Adapter<GifViewHolder>
		...
		public GifViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
			View view = activity.getLayoutInflater().inflate(
					R.layout.gif_list_item, parent, false);
		...
		public void onBindViewHolder(GifViewHolder holder, int position) {
			...
			holder.gifView.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View view) {
					...
					ClipData clip = ClipData.newPlainText("giphy_url",
							result.images.fixed_height.url);
					clipboard.setPrimaryClip(clip);

					Intent fullscreenIntent = FullscreenActivity.getIntent(
							activity, result);
