...
/**
 * A java wrapper for Giphy's http api based on
...
public final class Api {
	...
	private static String getSearchUrl(String query, int limit, int offset) {
		return signUrl(BASE_URL + SEARCH_PATH + "?q=" + query + "&limit="
				+ limit + "&offset=" + offset);
	}

	private static String getTrendingUrl(int limit, int offset) {
		return signUrl(BASE_URL + TRENDING_PATH + "?limit=" + limit
				+ "&offset=" + offset);
	}
	...
	public interface Monitor {
		 ...
		 * 
		 * @param result
		 *            The results returned from Giphy's search api.
		 */
	...
	/**
	 * A POJO mirroring the top level result JSON object returned from Giphy's
	 * api.
	 */
	...
	public static class GifResult {
		...
		@Override
		public String toString() {
			return "GifResult{" + "id='" + id + '\'' + ", url='" + url + '\''
					+ ", images=" + images + '}';
		}
	}

	/**
	 * A POJO mirroring a JSON object with a put of urls of different sizes and
	 * dimensions returned for a single image from Giphy's api.
	 */
	...
	/**
	 * A POJO mirroring a JSON object for an image with one particular url, size
	 * and dimension returned from Giphy's api.
	 */
	public static class GifImage {
		...
		@Override
		public String toString() {
			return "GifImage{" + "url='" + url + '\'' + ", width=" + width
					+ ", height=" + height + ", frames=" + frames + ", size="
					+ size + '}';
		}
	}
}
