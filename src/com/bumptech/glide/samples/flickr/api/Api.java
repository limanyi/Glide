...
public class Api {
	...
	private static final LruCache<UrlCacheKey, String> CACHED_URLS = new LruCache<>(
			MAX_URLS_TO_CACHE);
	...
	private static final List<Integer> SORTED_SIZE_KEYS = new ArrayList<>(
			EDGE_TO_SIZE_KEY.size());
	...
	private static String getSizeKey(int width, int height) {
		...
		String result = EDGE_TO_SIZE_KEY.get(SORTED_SIZE_KEYS
				.get(SORTED_SIZE_KEYS.size() - 1));
	...
	public static List<String> getAlternateUrls(Photo photo, int width,
			int height) {
		List<String> result = new ArrayList<>();
	...
	static String getSearchUrl(String text) {
		return getUrlForMethod("flickr.photos.search") + "&text=" + text
				+ PER_PAGE;
	}
	...
	public interface QueryListener {
		 ...
		 * @param photos
		 *            A list of images that were found for the given search
		 *            term.
		 */
		 ...
		 * 
		 * @param query
		 *            The query we attempted to obtain results for.
		 * @param e
	...
	public void query(Query query) {
		if (lastQueryResult != null && lastQueryResult.query.equals(query)) {
			for (QueryListener listener : queryListeners) {
				listener.onSearchCompleted(lastQueryResult.query,
		...
		FlickrQueryResponseListener responseListener = new FlickrQueryResponseListener(
				new PhotoJsonStringParser(), query, queryListeners);
		StringRequest request = new StringRequest(Request.Method.GET,
				query.getUrl(), responseListener, responseListener);
