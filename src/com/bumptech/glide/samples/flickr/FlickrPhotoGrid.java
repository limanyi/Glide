...
/**
 * A fragment that shows square image thumbnails whose size is determined by the
 * framgent's arguments in a grid pattern.
 */
public class FlickrPhotoGrid extends Fragment implements PhotoViewer {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		fullRequest = Glide.with(this).asDrawable()
		...
		thumbnailRequest = Glide
				...
				.apply(diskCacheStrategyOf(DiskCacheStrategy.DATA).centerCrop(
						getActivity()).override(Api.SQUARE_THUMB_SIZE));

		preloadRequest = thumbnail ? thumbnailRequest.clone().apply(
				priorityOf(Priority.HIGH)) : fullRequest;

		final View result = inflater.inflate(R.layout.flickr_photo_grid,
				container, false);

		final int gridMargin = getResources().getDimensionPixelOffset(
				R.dimen.grid_margin);
		int spanCount = getResources().getDisplayMetrics().widthPixels
				/ (photoSize + (2 * gridMargin));
		...
		grid.setRecyclerListener(new RecyclerView.RecyclerListener() {
			...
			public void onViewRecycled(RecyclerView.ViewHolder holder) {
				...
				Glide.with(FlickrPhotoGrid.this).clear(
						photoViewHolder.imageView);
			}
		});

		int heightCount = getResources().getDisplayMetrics().heightPixels
				/ photoSize;
		grid.getRecycledViewPool().setMaxRecycledViews(0,
				spanCount * heightCount * 2);
		grid.setItemViewCacheSize(0);
		adapter = new PhotoAdapter();
		grid.setAdapter(adapter);

		FixedPreloadSizeProvider<Photo> preloadSizeProvider = new FixedPreloadSizeProvider<>(
				photoSize, photoSize);
		RecyclerViewPreloader<Photo> preloader = new RecyclerViewPreloader<>(
				Glide.with(this), adapter, preloadSizeProvider,
	...
	private class PhotoAdapter extends RecyclerView.Adapter<PhotoViewHolder>
		...
		public PhotoViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
			View view = inflater.inflate(R.layout.flickr_photo_grid_item,
					parent, false);
		...
		public void onBindViewHolder(PhotoViewHolder holder, int position) {
			...
			fullRequest
					.load(current)
					.thumbnail(
							thumbnail ? thumbnailRequest.load(current) : null)
					.into(holder.imageView);

			holder.imageView.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View view) {
					Intent intent = FullscreenActivity.getIntent(getActivity(),
							current);
