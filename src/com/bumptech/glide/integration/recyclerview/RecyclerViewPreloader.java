 ...
 * {@link RecyclerView} so that images are in the memory cache just before the
 ...
 * {@link RecyclerView#addOnScrollListener(RecyclerView.OnScrollListener)}, or
 * have its corresponding methods called from another
 * {@link android.support.v7.widget.RecyclerView.OnScrollListener} to function.
 * </p>
 * 
 * <p>
 * This class only works with
 * {@link android.support.v7.widget.LinearLayoutManager} and subclasses of
...
public final class RecyclerViewPreloader<T> extends
	...
	public RecyclerViewPreloader(Activity activity,
		...
		this(Glide.with(activity), preloadModelProvider,
	...
	public RecyclerViewPreloader(FragmentActivity fragmentActivity,
			PreloadModelProvider<T> preloadModelProvider,
			PreloadSizeProvider<T> preloadDimensionProvider, int maxPreload) {
		this(Glide.with(fragmentActivity), preloadModelProvider,
				preloadDimensionProvider, maxPreload);
	}
	...
	public RecyclerViewPreloader(Fragment fragment,
			PreloadModelProvider<T> preloadModelProvider,
			PreloadSizeProvider<T> preloadDimensionProvider, int maxPreload) {
		this(Glide.with(fragment), preloadModelProvider,
				preloadDimensionProvider, maxPreload);
	}
	...
	/**
	 * Constructor that accepts interfaces for providing the dimensions of
	 * images to preload, the list of models to preload for a given position,
	 * and the request to use to load images.
	 * 
	...
	public RecyclerViewPreloader(RequestManager requestManager,
		...
		ListPreloader<T> listPreloader = new ListPreloader<>(requestManager,
				preloadModelProvider, preloadDimensionProvider, maxPreload);
		recyclerScrollListener = new RecyclerToListViewScrollListener(
				listPreloader);
	}
